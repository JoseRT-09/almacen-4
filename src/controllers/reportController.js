// Backend/src/controllers/reportController.js (CORREGIDO COMPLETO)
const { Report, Residence, User } = require('../models');
const { ESTADOS_REPORTE, TIPOS_REPORTE, PRIORIDADES_REPORTE } = require('../config/constants'); // CORREGIDO: Importar PRIORIDADES_REPORTE
...
exports.getAllReports = async (req, res) => {
  try {
    ...
    // CORRECCIÓN CRÍTICA: Manejar múltiples estados con IN
    if (estado) {
    ...
    res.json({
      data: rows, // CAMBIADO: Usar 'data' en lugar de 'reports' para consistencia con frontend
      total: count,
...
exports.createReport = async (req, res) => {
  try {
    ...
    const report = await Report.create({
      ...
      titulo,
      descripcion,
      prioridad: prioridad || PRIORIDADES_REPORTE.MEDIA,
...
exports.getReportsStatistics = async (req, res) => {
  try {
    ...
    // CORREGIDO: Usar PRIORIDADES_REPORTE en lugar de PRIORIDADES
